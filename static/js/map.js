...
let osmUrl = 'http://{s}.tile.osm.org/{z}/{x}/{y}.png',
    ...
    osm = L.tileLayer.grayscale(osmUrl, {attribution: osmAttrib, opacity: 0.55}),
...
// Attivazione plugin per gestione marker sovrapposti
...
async function searchPlaces() {
    ...
    try {
        ...
        setTimeout(function () {
            ...
            try {
                // Aggiusta mappa per fittare tutte le biblioteche
            ...
            setTimeout(function () {
                ...
                // Attiva automaticamente popup biblioteca se il risultato è uno solo
                setTimeout(function () {
                    let opened = false;
                    layers.forEach(function (layer) {
                        if (!opened && layer instanceof L.Marker && response.data.length === 1) {
                            ...
                            setTimeout(function () {
                                ...
                                // Attiva automaticamente l'espansione se il record è uno solo
                                setTimeout(function () {
...
// Genera marker biblioteca con badge numero
function generateNumberedBiblioMarker(biblio) {
    ...
    // Usa un font più piccolo nel badge per numeri con > 2 cifre
    ...
    // Direziona il tooltip a sinistra/destra in base alla biblioteca
    ...
    let icon2 = L.marker(coords, {
        icon: L.divIcon({
            ...
            iconSize: [22, 22], iconAnchor: [0, 42], popupAnchor: [-3, -30]
        }),
        bounceOnAdd: true,
        bounceOnAddOptions: {duration: rDuration, height: rHeight, loop: rLoop}
    });
    ...
    // Genera contenuto popup biblioteca
    ...
    biblio.records.forEach(function (r) {
        ...
        $(document).on('click', '#' + r.record_id, function () {
            ...
            // Richiedi espansione e mostra curve sulla mappa
...
function fetchFullRecordAndShowCurves(id) {
    ...
    axios.get(`/api/v1/get_full_record?record_id=${id}`).then(function (r) {
        ...
        // Popola la sidebar con i dati del record
        setUpSidebar(r.data);

        if (expansionsEnabled) {

            // Rimuovi layer precedenti dalla mappa
            ...
            // Genera marker luogo di pubblicazione
            ...
            // Genera marker per entità
            record.entities.forEach(function (entity) {
                if (entity.title !== 'Firenze') { // Non mostrare Firenze come entità perchè è troppo vicina alla biblioteca e viene male

                    // Tieni tracca di eventuali entità multiple con la stessa coordinata
                    ...
                    // Aggiungi freccia se l'entità non è contenuta in Toscana
            ...
            // Scrivi 'Entità multiple' sull'arco in caso di entità multiple con la stessa coordinata
            if (dupeCoord !== '') {
                let i = 0;
                ...
                // Controlla se ci sono entità molto vicine tra loro e nel caso non scrivere testo sull'arco, per evitare collisioni

                let minDistance = 10000; // 10km
                let done = [];
                    ...
                    if (!done.includes(s1)) {
                        ...
                        coords.forEach(function (c2) {
                            ...
                            if (!done.includes(s2)) {
                                ...
                                if (map.distance(c1, c2) < minDistance) {
                                    toAdd.filter(function (value) {
            ...
            let realGroup = L.featureGroup(realLay); // Gruppo che contiene solo i marker che rientrano in Toscana
            let group = L.featureGroup(lay); // Gruppo con tutte le entità, anche quelle lontane

            // Fitta la mappa in base alle entità in Toscana
            map.fitBounds(realGroup.getBounds(), {paddingTopLeft: [50, 230], paddingBottomRight: [50, 50]});

            // Aggiunge tutti marker e le curve sulla mappa
            setTimeout(function () {
                group.addTo(map);
                isFetching = false;
            }, 600);
...
// Genera marker per luogo di pubblicazione
...
// Centra la mappa ma con un offset
...
function clickZoom(marker) {
    ...
    // Centra il marker ma con un offset, nel caso in cui la sidebar sia aperta
...
// Genera HTML per i risultati dei record nel popup biblioteca
...
// Genera un intorno di 4 parole a destra e a sinistra della query a partire dal testo 'text'
...
// Ritorna true se esiste un marker con lo stesso nome e dello stesso tipo sulla mappa
...
// Ritorna true se esiste un marker con lo stesso nome ma di tipo diverso sulla mappa
...
// Pulisce la mappa da qualsiasi layer
...
// Genera una curva di Bezier con punto di controllo automatico (da https://gist.github.com/ryancatalani/6091e50bf756088bf9bf5de2017b32e6)

function bezierCurve(from, to, color, dash, text, needsArrow) {
    // Determina automaticamente il punto di controllo
    ...
    let midpointLatLng = [midpointY, midpointX]; // punto di controllo

    let pathOptions = {color: color, animate: 600, weight: needsArrow ? 0 : 1, dashArray: dash ? 5 : 0};

    // Genera una curva quadratica di Bezier
    let curve = L.curve(['M', from, 'Q', midpointLatLng, to], pathOptions);

    // Estrai una approssimazione della curva usando 100 punti lat/lng
    let i,
        points = [];
    let precision = 100;
    ...
    // Aggiungi sulla mappa una polyline 'dummy' che imita la curva, con opacità zero per mostrare soltanto il testo sull'arco
    // Questo perchè ad oggi la libreria TextPath non supporta L.Curve ma solo le Polylines
    ...
    // Aggiungi sulla mappa una seconda polyline.
    // Se l'entità è in Toscana, questa seconda Polyline avrà opacità zero.
    // Se invece l'entità è fuori dalla Toscana (needsArrow = true), viene mostrata questa seconda polyline con opacità 100%
    // al posto della L.Curve. Questo perchè la freccia sull'arco funziona solo con la Polyline e non con la curva originale.
    ...
    dummyPolyline2.setText('  ►  ', {center: true, offset: 7, allowCrop: false, edgeArrow: true, clickable: true, attributes: { 'font-size': '20' } }).on('click', function () {
        ...
        // Se clicco la freccia la mappa viene fittata per contenere l'entità lontana, e il suo popup viene aperto automaticamente
        let group = L.featureGroup([L.marker(from), L.marker(to)]);
        map.fitBounds(group.getBounds(), {paddingTopLeft: [50, 150], paddingBottomRight: [50, 50]});
        setTimeout(function () {
            let found = false;
    ...
    curve.on('add', function () {
        setTimeout(function () {
            ...
            if (needsArrow) {
                ...
                // Aumenta il range di tocco della freccia, utilizzando la libreria TouchHelper
...
// Equazione curva quadratica di Bezier
